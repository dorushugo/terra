...
import { MotionGrid } from '@/components/ui/MotionTransitions'
...
export const LoadMoreRelatedProducts: React.FC<LoadMoreRelatedProductsProps> = ({
  initialProducts,
  currentProduct,
  totalProducts,
  className = '',
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to load more related products')
      }
  ...
  return (
    <div className={`space-y-8 ${className}`}>
      <MotionGrid className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 gap-6 md:gap-8">
      ...
      {!hasMore && products.length > 9 && (
        <div className="text-center">
          <p className="text-gray-600 font-terra-body">Vous avez vu tous les produits similaires</p>
        </div>
      )}
    </div>
  )
